...
- next-intl: a message key must NEVER contain a `.` (`"meetup.proposed"` → nest it as `meetup: { proposed }`); the whole namespace is refused at RENDER time and the CI i18n script (parse + FR/EN mirror) cannot see it (#174).
- Every service needs a versioned `src/assets/.gitkeep`: the Nx `build` target declares `assets: ["./src/assets"]`, webpack fails with `ENOENT` if the folder is missing, `nx serve` is skipped and the gateway answers 500 — the CI runs `tsc` + `jest`, never `nx build` (#174).
