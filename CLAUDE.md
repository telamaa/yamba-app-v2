...
Test platform baseline: **621 tests** (trip-service 198, deal-service 355, notification-service 68) + auth-service 59 (also a CI check) — any deviation must be explained.
...
- LAN testing (other computers/phones on `http://192.168.x.x:3000`): switch `NEXT_PUBLIC_API_BASE_URL` in `apps/user-ui/.env.local` to the LAN IP and restart user-ui; `allowedDevOrigins` in `next.config.js` is required or Next 16 answers 403 on `/_next/*` and pages stay on their SSR skeleton.
