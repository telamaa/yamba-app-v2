...
npx nx serve message-service       # port 6005 (chat + rendez-vous, chantier F — outbox relay, Redpanda optionnel)
...
Test platform baseline: **1170 tests** (trip-service 310, deal-service 659, notification-service 122, message-service 79) + auth-service **406** (also a CI check) — any deviation must be explained. Measured 19/09/2026, after A201 (session mobile par tokens, auth +6) ; le 18/09 : D79 (fiche membre « Communications », auth +5), trip était resté noté 308 alors que la mesure disait 310 depuis la passe du 18/09. Historique proche : les quatre campagnes de recette, D78, les trois passes de concurrence (A195/A196/A197), le déterminisme A199 et les six arbitrages A198.
...
- Base branch `dev`, protected by required status checks — **18** since 18/09/2026 (TypeScript **×8**: user-ui, admin-ui and the six services, unit tests ×5: auth/deal/notification/trip/message, webpack build of the six services, i18n FR/EN mirror + no-dot keys, secrets anti-leak, OpenAPI contracts generate+diff, and **TypeScript (fiches de test)**). The count was written ×9 for TypeScript: it has always been ×8 — corrected 18/09. The last one is the one that sees the **spec files**: the per-project `tsconfig.app.json` jobs EXCLUDE `src/**/*.spec.ts`, so a defect living only in a test file was invisible to the whole CI (measured 18/09: `nx typecheck auth-service` at 53 errors while CI was green). It runs `npx nx run-many --target=typecheck --all --skip-sync`; `--skip-sync` is deliberate — applying the Nx sync generator adds a reference from `apps/e2e` to `apps/admin-ui`, which is `noEmit: true`, and a referenced project may not disable emit (TS6310). Never commit directly to `dev`: `feat/*` or `chore/*` branch + PR.
...
- Never `npm audit fix --force` inside a feature PR (dedicated `chore/deps` PR). Toolchain since 05/09/2026: Nx 23.2, TypeScript 6.0 (`ignoreDeprecations: "6.0"` in every tsconfig), `overrides` in the root package.json for `uuid` / `deepmerge-ts` / `qs`. **`npm audit` is NOT at 0** — measured 18/09/2026: **18** (1 moderate, 17 high), and the line saying it was expected at 0 had gone stale. Most of them are the Nx family, whose "fix" is a **downgrade** to `nx@22.6.4` (`isSemVerMajor`) from the 23.2 in use — the exact same trap as `imagekit`, which stays 6.0.0 (the audit's "fix" downgrades it to the 2016 fossil). **Never `npm audit fix --force`.** The only genuine, non-major fixes pending: `nodemailer` 9.0.3 → 9.1.1 and `morgan` → 1.12 (plus `svgo`, transitive) — for a dedicated `chore/deps` PR.
...
- `packages/libs/email` — shared transactional mailer (D41/D44/D35): an `EmailProvider` behind `getEmailProvider()` — `resend` (HTTP via `fetch`, no SDK), `smtp` (Nodemailer, Mailpit locally), `fake` (in-memory, REFUSED in production; `resolveEmailProviderName` from `EMAIL_PROVIDER` / `RESEND_API_KEY` / `SMTP_HOST`) — `sendTemplatedEmail` (per-service EJS files, legacy) and `sendTransactionalEmail` (ONE embedded EJS layout string + `EmailContent` data) both return `{ provider, providerMessageId }` and accept `tags` + `idempotencyKey`. Webhooks (Svix signature, `verifySvixSignature` + pure `interpretEmailEvent`) land in notification-service `/webhooks/email/resend` and update `EmailDelivery`; a hard bounce or complaint sets `User.emailSuppressedAt` — EVERY recipient resolver must skip `isDeleted` AND `emailSuppressedAt` (a new email flow that forgets it is a bug). The trip-service mailer delegates to this lib. auth-service emails are per-locale dictionaries (`apps/auth-service/src/emails/auth-emails.ts`). The locale list lives ONLY in `packages/libs/api-contracts/src/locale.ts` (`SUPPORTED_LOCALES`, `resolveLocale`); the front consumes it through the dedicated alias `@packages/api-contracts/locale`. Never write a new `fr ? … : …` — add a dictionary entry. Email language = the RECIPIENT's `User.preferredLocale`; account-less flows use the request locale (`x-locale` header sent by the API clients).
...
- `packages/libs/csv` — safe CSV export (C-PR7a, D60 2A): `csvCell` (RFC 4180 + formula-prefix neutralisation), `buildCsv(columns, rows)`, `CSV_BOM`; every admin export is journaled (`EXPORTED`), personal-data exports (users) need `exports.personal` — SUPER_ADMIN **or PRIVACY** (A143) — with a ≥ 20-char reason, operational exports (`exports.operational`: trips, tickets, arbitration) carry ids only, never email/phone.
- `packages/libs/settings` — platform settings reader (C-PR8a, D62): ONE Mongo document `PlatformSettings` (key `current`, `values` JSON, `version` optimistic lock), `makeSettingsReader` (30 s in-process cache, fail-safe: unreadable/absent document → catalogue defaults, never throws), `default.ts` = the Prisma-bound singleton `platformSettings()`. The catalogue (keys, bounds, defaults, scope, texts) lives in `packages/libs/api-contracts/src/admin/platform-settings.schema.ts` and is the single source for the admin page, its tooltips, OpenAPI and the generated `context/YAMBA-PARAMETRES.md` (`npm run settings-doc`). Rewiring rule: a pure rule keeps its constant as the DEFAULT ARGUMENT, services read `await settings.get()`; a catalogue key must have a real consumer (otherwise class C). Writes go through auth-service `/admin/settings` only (reason ≥ 20, scope per key: BUSINESS = SUPER_ADMIN, OPERATIONS = OPS, one `SETTING_CHANGED` journal line per key, email to every SUPER_ADMIN). Never retroactive (booking snapshot). QA reset: `seed-settings.ts` (`--show` to inspect).
- `packages/libs/health` (uniform `/health`: `healthHandler(service, { mongo, redis })`, 2 s per check, always HTTP 200 with `ok` / `degraded`), `packages/libs/redis/cron-heartbeat.ts` (`withHeartbeat` around EVERY cron tick → `yamba:cron:<service>:<name>`, 7-day TTL, best effort; a new cron MUST be wrapped or it is invisible on the admin status page; D70: `withHeartbeat` also pings the external monitor URL declared in `CRON_HEARTBEAT_PING_URLS` `{"<service>:<cron>": url}`, best effort). Public uptime probe (D70): gateway `GET /api/status` declared BEFORE the rate limiter, 10 s cache, 200 = ok / maintenance, 503 = degraded / down, built from `packages/libs/health/status.ts` (`serviceEntries`, `probeService`, `aggregateStatus` — the admin status page reuses them); fronts answer `GET /api/health`, `packages/libs/retention` (pure retention rules, `retention.*` settings, a parked outbox event is never purged), `packages/libs/maintenance` (pure read-only-mode rules; the gateway polls the `maintenance` PlatformSettings document every 10 s, `MAINTENANCE_MODE=on` overrides, writes answer 503 `MAINTENANCE` except `/api/auth/*` and `/api/admin/*`) — C-PR8c, D64. The gateway now has the generic `@packages` webpack alias and the services' tsconfig layout.
- `packages/libs/analytics` — PostHog server capture (D66): `analyticsEventsFor(outboxEvent, consentingUserIds)` is a WHITELIST projection (never a payload spread — recipient, delivery code, emails must never reach PostHog), `captureServerEvents` posts `/batch` via `fetch`, inert without `POSTHOG_API_KEY`, never throws; called by the notification-service consumers (`lib/analytics-sink.ts`) only for users with `analyticsOptIn: true`. Front (`apps/user-ui/src/lib/analytics.ts`): `posthog-js` is dynamically imported ONLY after the consent banner is accepted; `track()` is the single entry point for funnel events; `identify` uses the user id only.
- `packages/libs/imagekit` — shared ImageKit client (lazy proxy, `deleteImageKitFile` tolerates « does not exist »); the trip-service `lib/imagekit.ts` is a shim. GDPR (C-PR8b, D63): member export/erasure live in auth-service `privacy.service.ts` (sudo = the `sudo` OTP scope by email; erasure = ONE transaction that anonymises `User` field by field — uniques become `erased+<id>@anonymised.invalid` / `deleted-<id>`, NEVER null — keeps bookings/disputes/reviews/messages, moves `stripeAccountId` to `ErasedAccount`, writes `DataRequest`; blocked by any live deal with a CLOSED list of `ErasureBlocker`s). After erasure `isDeleted` is what every email resolver and `isAuthenticated` must check (the technical email is not empty). Recipient snapshots are redacted by a deal-service cron after `privacy.recipientRetentionDays`.
- `packages/libs/trust` — internal TrustScore (D71, D29 ②): `computeTrustScore(signals)` pure (0..100, NEW / STANDARD / WATCH / HIGH_RISK, readable factors), `checkCaps` (CNF-06 progressive caps → deal-service 409 `NEW_ACCOUNT_CAP` at payment-intent AND booking creation), `loadTrustSignals(db)` (denormalised counters + 4 counts, Prisma injected). Computed on READ, never stored, never served to a member, never an automatic sanction; the admin user file and the reports queue read it.
...
`apps/admin-ui` (port 3001, FR only, no next-intl) talks to the gateway through the same `/api` proxy. Admin sessions are SEPARATE from user sessions: cookies `admin_access_token` / `admin_refresh_token` / `admin_preauth`, Redis prefix `admin_jti:`, JWT claims `adm: true` + `amr: ["pwd","totp"]`, middleware `packages/middleware/isAdminAuthenticated.ts` (never reads `access_token`). Login is two-step (`POST /auth/admin/login` → `totp/verify` or `totp/setup` + `totp/enable`). The ADMIN role is only granted by `packages/libs/prisma/scripts/grant-admin.ts <email> --role <SUPER_ADMIN|MEDIATOR|SUPPORT|FINANCE|OPS|PRIVACY>` or by a SUPER_ADMIN invitation (D56): `User.adminRoles` (list, C-PR3bis / D60 1A: cumulated profiles = UNION of permissions, `adminRole` kept as the primary-role mirror; writers go through `apps/auth-service/src/utils/admin-roles.ts`, never leave the list absent; `backfill-admin-roles.ts` for pre-existing accounts) is REQUIRED to open an admin session, and route permissions come from `ADMIN_PERMISSIONS` in `packages/libs/api-contracts/src/admin/admin-users.schema.ts` (middleware `requireAdminPermission`). Account sanctions (`User.accountStatus` RESTRICTED / SUSPENDED) take effect through READS only: `isAuthenticated` (suspended → 401), `requireActiveAccount` on trip/booking creation, and a search filter on `user.accountStatus` — never a cross-service write. Admin routes live in the owning service (`/admin/disputes/*` in deal-service, `/admin/me`, `/admin/audit` in auth-service, `/admin/conversations/*` in message-service — F-PR3 D61 7A: journaled thread reading, reported-messages queue; gateway proxies `/api/admin/conversations` to `:6005`); gateway proxies `/api/admin/disputes` + `/api/admin/finances` + `/api/admin/deals` + `/api/admin/alerts` (C-PR6b, D59: threshold alerts computed on read from `apps/deal-service/src/services/ops-alerts.rules.ts`, hourly cron emails support once per rule per day via a Redis NX key) to deal-service (C-PR5, D58: exception queues, money file of any deal, read-only provider reconciliation via `PaymentProvider.inspect`, payout retry with spaced retries `payoutNextRetryAt`, reversal resolution; C-PR5b: monthly report + CSV export computed from bookings, manual refund proposed by FINANCE/SUPPORT and applied by SUPER_ADMIN only, money first then one conditional transaction), `/api/admin/trips` + `/api/admin/tickets` to trip-service (C-PR4, D57: ticket queue, « masqué par Yamba » = `Trip.hiddenByAdminAt` read by search / public page / booking creation), the rest of `/api/admin/*` to auth-service (`/admin/kpis`, `/admin/pilotage/*` included — C-PR6a, D59: series and corridors computed server-side with a 60 s Redis cache; `/admin/deals/:id/history` in deal-service merges outbox + journal + notifications + emails read-only). Demand counters (trip page views deduplicated per visitor/day, searches per corridor incl. no-result) live in Redis via `packages/libs/redis/trip-stats.ts` (pure key builders, written by trip-service, read by auth-service).
...
- Prisma+Mongo: `field: null` in a `where` misses ABSENT fields → `OR: [{field: null}, {field: {isSet: false}}]` — **but `isSet` only exists on OPTIONAL fields**: on a REQUIRED field with a default (`isDeleted`, `profilePublic`), an absent value is unmatchable by any filter (`not` included) and is a DATA defect — run `packages/libs/prisma/scripts/repair-absent-scalars.ts` after adding such a field (ANO-API-23: the public trip page 404'd on 24 trips out of 37, the public profile on 22 accounts out of 26) — for EVERY nullable filter (`readAt`, `publishedAt`…), and writers must set `null` explicitly. Paid SIX times (readAt, reservedKg A34, outbox relay A49, trackingEvents A85, Dispute.resolvedAt C-PR2, User.profilePublic + Trip.isDeleted ANO-API-23): a fixture that sets the field proves nothing about the real writer. **Composite/scalar LISTS are worse: no Prisma filter (`none`, `some`, `isEmpty`, `equals: []`) matches an ABSENT list** — writers must create lists as `[]` (booking-request.ts does), concurrency guards go through `updatedAt` (optimistic lock), and `packages/libs/prisma/scripts/repair-absent-lists.ts` back-fills existing documents.
...
- TypeScript 6 migration (Nx 23, 05/09/2026) wrote `esModuleInterop: false` into `tsconfig.base.json` to "preserve" the TS 5 default — but under `module: nodenext` the effective value WAS true: `tsc` and the tests (their `tsconfig.spec.json` set it) stayed green while every webpack bundle crashed at boot with `(0, express_1.default) is not a function`. `esModuleInterop` must stay `true` in `tsconfig.base.json`; a green build is NOT a booted service — run `bash scripts/smoke-services.sh` after any toolchain upgrade (it boots the six bundles on ports +900 and reads their `/health`; note the port variable differs: `PORT` for gateway / auth / trip / deal, `NOTIFICATION_SERVICE_PORT`, `MESSAGE_SERVICE_PORT`). Nx 23 `@nx/js:node` opens the inspector on 9229 for EVERY service: each serve target now carries its own `port` (gateway 9230, auth 9231, trip 9232, deal 9233, notification 9234, message 9235).
...
- The gateway CORS allow-list is by port (3000 user-ui, 3001 admin-ui) on localhost / 192.168.x.x / 10.x.x.x — the browser Origin header crosses the Next proxy (D48), so a new front app or port must be added there or every call fails 500 "Not allowed by CORS" (recette C-PR1, #151).
- `overflow-x: clip` (not `hidden`) to preserve `position: sticky`.
- next-intl: a message key must NEVER contain a `.` (`"meetup.proposed"` → nest it as `meetup: { proposed }`); the whole namespace is refused at RENDER time; the CI i18n script now refuses such keys at the source (#174).
- Every service needs a versioned `src/assets/.gitkeep`: the Nx `build` target declares `assets: ["./src/assets"]`, webpack fails with `ENOENT` if the folder is missing, `nx serve` is skipped and the gateway answers 500 — since the CI build job (chore/ci-build-i18n) this is caught before merge (#174).
