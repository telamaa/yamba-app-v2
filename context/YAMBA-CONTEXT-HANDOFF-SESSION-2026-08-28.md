...
- **Fix relay (A49)** : aucun email/notification en recette → relay `publishedAt: null` ne voit pas les champs ABSENTS (3e occurrence du pitfall) ; filtre OR + writers explicites, 38 orphelins parqués. **À faire utilisateur** : copier `SMTP_*` de `apps/trip-service/.env` dans `.env` racine, relancer deal-service ET notification-service.
