...
31. **F-PR1 `feat/f1-message-service` — FAIT, mergée #169 (16 checks : le service rejoint les matrices CI)** (D61, A132–A134 ; challenge du chat requalifié en « coordination » le 04/09, puis « service dédié dès maintenant » décidé par l'utilisateur). Livré : nouveau **message-service (6005)** — `Conversation` par deal ouverte à l'acceptation, **`Meetup` objet** (propose / accepte / contre-propose, verrou optimiste), fil avec deux gardes (**code de livraison refusé** par comparaison bcrypt sur les groupes de six chiffres, **coordonnées signalées sans blocage**), **numéro révélé au plus tôt 2 h avant le rendez-vous** et tracé, réponses rapides FR/EN, `GET/POST /messages/*` sous `isAuthenticated`, **relais outbox dédié** (topic `messaging-events`, bail propre) et **filtre `aggregateType: "booking"` ajouté au relais du deal-service** (A132 — sans lui, les événements de conversation auraient été parqués), OpenAPI **4e document** (script + diff CI), service ajouté aux matrices CI (typecheck et tests), seed d'un fil sur `bzv-accepted`. **Gestes utilisateur** : `npx prisma db push`, relancer le gateway et lancer `npx nx serve message-service`, rejouer `seed-deals.ts`, jouer FCH01–FCH10. **Attention ordre de fusion** : #168 (C-PR6b alertes) est verte et non fusionnée ; cette branche a été pré-fusionnée avec elle. Suite : **F-PR2** front des deux rôles + notifications (in-app immédiate, email de relance à 15 min), **F-PR3** admin (lecture depuis un dossier, signalement) et purge à un an ; puis C-PR8 paramètres / RGPD.
32. **F-PR2 `feat/f2-messaging-front` — FAIT, mergée #170** (D61, A135–A136 ; « Go pour F-PR2 »). Livré : messagerie du tableau de bord (liste des fils, conversation groupée par jour, panneau rendez-vous avec proposer / contre-proposer / accepter, réponses rapides, bouton du numéro avec heure d'ouverture, saisie fermée avec motif), bulle du header branchée sur `totalUnread` (elle était figée à 3), namespace i18n `messaging` FR/EN, **sondage adaptatif** 3 s sur un fil ouvert et 20 s sur la liste (A135 — pas de temps réel tant que les seuils de D61 ne sont pas atteints), **notification in-app** par un second consumer (topic et groupe dédiés, 5 tests). **Écart annoncé** : l'email de relance des messages non lus (D61 6A) part en F-PR3 — un email annonçant un message qu'on ne peut pas ouvrir serait cassé. **Gestes utilisateur** : relancer le gateway, `npx nx serve message-service` (inclus dans `npm run dev`), user-ui et notification-service ; Redpanda requis pour la notification in-app ; jouer FCH11–FCH16. Suite : **F-PR3** (email de relance, lecture admin depuis un dossier, signalement d'un message, purge à un an), puis boutons « Message » sur les écrans de deal, puis C-PR8 paramètres / RGPD.
33. **F-PR2b `feat/f2b-deal-message-entry` — PR ouverte** (A137 ; « attaque avec les prochaines PR après »). Livré : les **sept boutons « Message » / « Appeler »** des écrans de deal (tracker Expéditeur accepté / en transit ; deal Voyageur accepté / remise / suivi / livraison), qui faisaient un `console.info` depuis B3/B4, ouvrent le fil du deal par `useOpenDealThread` (`by-deal` → cache pré-rempli → `/dashboard/messages?conversation=<id>`) ; **« Appeler » ne compose jamais** : `?focus=phone` et un bandeau dans le fil (numéro `tel:` si révélé, « Voir le numéro » si la fenêtre H-2 est ouverte, sinon l'heure ou l'invitation à confirmer un rendez-vous). 403 → toast « la conversation s'ouvre une fois le deal accepté ». Aucun test serveur touché. **Gestes utilisateur** : relancer user-ui ; jouer FCH17–FCH19. Suite : **F-PR3** (email de relance, lecture admin depuis un dossier, signalement, purge), puis C-PR8.
34. **F-PR3 `feat/f3-messaging-admin` — PR ouverte** (D61 6A/7A/8A, A138–A141). Livré : **relance email** des messages non lus (règle pure `unreadReminderDue` : 15 min, autre partie seulement, une par heure et par conversation ; cron 5 min ; verrou optimiste `updateMany` sur `<role>RemindedAt` ; email par locale sans le texte du message ; `MESSAGING_REMINDER_CRON_ENABLED`), **lecture admin** d'un fil entier depuis la fiche litige ou la file (`GET /admin/conversations/by-deal/:bookingId`, permission `conversations.read` MEDIATOR + SUPPORT, journal `CONVERSATION_VIEWED`, jamais le numéro), **signalement** d'un message texte de l'autre partie (drapeau sur la bulle, `Report` MESSAGE, 409 doublon, sans outbox — A140), **file `/reports`** (`reports.review`, « Traité » / « Sans suite » + note, décision et `MESSAGE_REPORT_REVIEWED` dans la même transaction), tuile d'accueil `messageReportsOpen`, **purge à un an** (`isPurgeable`, cron 03:30, signalements conservés, `MESSAGING_RETENTION_CRON_ENABLED`). message-service **28** tests (+14), OpenAPI 4 documents régénérés, seed : message signalé sur `bzv-accepted`. **Gestes utilisateur** : `npx prisma generate` + `npx prisma db push` (nouveaux champs, enum), relancer gateway / auth-service / message-service / les deux fronts, rejouer `seed-deals.ts`, jouer FCH20–FCH26 (FCH20–22 demandent SMTP). Suite : **C-PR8** paramètres audités (seuils d'alerte et de relance, commission), RGPD export / effacement, maintenance.
