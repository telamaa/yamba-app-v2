...
10. **B4-PR1 `feat/b4-payout-server` — FAIT, en PR** (D49–D52, A65–A70). Décisions utilisateur du 03/09 après challenge métier/UX : 1A (litige « non livré » depuis PICKED_UP, **48 h** après le départ) · 2B (DISPUTED terminal, **chantier C lancé juste après B4**, pas de route admin jetable) · 3 (`source_transaction`) · 4A (retenue ANN-01 au prorata de la part nette, PR dédiée `feat/b4-late-cancel-payout`) · 5 à 10 oui (rappel J+3, confirmation en bouton secondaire, copie honnête 2–7 jours, état « versement en attente : finalise ton compte Stripe », catégorie du litige au Voyageur, aucun CTA « Noter » avant B5). **Méthode exigée par l'utilisateur : challenger chaque lot en expert métier et UX/UI et soumettre les points AVANT de coder.** `prisma db push` et seed rejoués (2 dossiers `Dispute`). Suite : **B4-PR2 front Expéditeur** (bascule des mocks `confirmDeliveryEarly` / `submitDispute` de `booking-tracker.api.ts`, vues COMPLETED / DISPUTED, upload photos litige `deals/dispute/` via `useImageKitUpload`, confirmation anticipée en bouton secondaire avec avertissement définitif) → **B4-PR3 front Voyageur** (états `payoutStatus`, échec avec CTA onboarding Stripe, `disputeCategory`) → retenue ANN-01 → chantier C.
