...
25. **C-PR5 finances — challenge posé le 04/09, en attente des décisions.** Inventaire : tout l'argent vit sur le `Booking` (snapshot, intent / charge, remboursement sans `refundId`, versement PENDING / SENT / FAILED / FROZEN / REVERSED, retenue), exécuteur unique + cron 5 min plafonné à 100 tentatives puis silence, REVERSED seulement loggé, digest email quotidien au support, remboursement « argent d'abord » (D39) sans détection d'un remboursement parti sans écriture, profil FINANCE sans écran. Décisions proposées (A recommandé) : 1 périmètre sans grand livre · 2 trois files (échecs, renversés, retenues) · 3 gestes rejeu / clôture d'un renversement / remboursement manuel (SUPER_ADMIN applique) · 4 fiche argent de tout deal + rapprochement Stripe à la demande · 5 rapport mensuel par devise + export CSV journalisé · 6 permissions `finances.*`, `payouts.*`, `refunds.manual.*` · 7 digest conservé · 8 deux PR (C-PR5a files / fiche / rejeu, C-PR5b rapport / export / remboursement manuel). Axes non demandés : stocker `refundId` / `transferId`, deals à marge négative (frais Stripe), espacement croissant des rejeux, devises dès le premier rapport, tuile « Versements en échec » → `/finances`. Dès les réponses : graver D58, brancher `feat/c5a-admin-finances` depuis `dev`.
