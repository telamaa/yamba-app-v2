...
| fix · `fix/session-expired-gate-provider` — fenêtre « session expirée » (A89) montée sous les providers (crash `useUiPreferences`), `turbopack.root` | 🔄 PR #163 ouverte |
| C · `feat/c6c-admin-pilotage-v2` — **D60 3A / 4A, A120–A122** : courbes agrandies (tableau, drill-down par point, borne 200, journal des inscriptions), onglet Finances du pilotage, pastille « n vues » + badge « Populaire » (cartes desktop / mobile, détail public) ; auth 96 | 🔄 PR ouverte |
...
| Indicateur | Valeur |
|---|---|
| Tests (plateforme) | 762 = trip 207 · deal 477 · notification 78 (+ auth-service 96, check CI « Tests unitaires (auth-service) ») |
| Décisions au registre | D1 → D60 (+ arbitrages A1 → A122) |
