...
| F · `feat/f2b-deal-message-entry` — **A137** : les sept boutons « Message » / « Appeler » des écrans de deal ouvrent le fil du deal (`by-deal` + cache pré-rempli + `?conversation=`), « Appeler » = fil avec `?focus=phone` (bandeau numéro / heure d'ouverture), i18n `messaging.open.*` / `phone.banner.*` | 🔄 PR ouverte |
