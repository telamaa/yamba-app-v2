...
| Élément | Statut |
|---|---|
| Chantier C admin-ui : médiation litiges (tickets YAM-XXXX), vérification billets, file des signalements, gestion users, **paramètres plateforme audités** (D62), RGPD export / effacement (D63), maintenance / état des services / conservation (D64), profils cumulés SUPER_ADMIN · MEDIATOR · SUPPORT · FINANCE · OPS · PRIVACY, login séparé, 2FA TOTP, audit log | ✅ **SOLDÉ le 05/09** (C-PR1 #148 → C-PR8c #182) — reste hors chantier : TrustScore interne + plafonds progressifs (D29-2) |
...
| Micro-PRs confiance : ~~wording statuts D28, bouton Signaler (trajet + membre), CTA alertes~~ FAITS (D68) · page destinataire (D69, lot 2) | 🟡 |
...
| Copie · `chore/vocabulaire-voyageur` — **A90** : « Voyageur » (FR) / « Traveler » (EN) à la place de Yamber / Tripper dans toute l'interface (57 valeurs, 11 fichiers, 8 chaînes de code) ; identifiants et route `/become-yamber` inchangés | ✅ mergée #141 |
| UX · `feat/notifications-vivantes` — **A91** : polling 30 s + focus, copie par événement et rôle avec prénom servi (`counterpartFirstName`), cloche desktop = menu 5 dernières + « Tout marquer lu » (`PATCH /me/notifications/read-all`), email d'atterrissage (matrice A35 amendée), contrat notification accepte les types système ; notification 77 | ✅ mergée #142 |
| B5 · `feat/b5-rating-server` — **D53 / A92–A94** : `GET/POST /deals/:id/rating` (double-aveugle en transaction, `canRate` machine, verrou `updatedAt`, unicité de service), cron horaire (relances J+5/J+7, révélation à 14 j), `reputation.service` (niveaux NEW/CONFIRMED/TOP dénormalisés, `isSuperCarrier` = TOP), profil public (réputation + avis révélés seuls), email de rappel + bouton « Noter » de retour ; deal 417 · notification 78 | ✅ mergée #144 |
| B5 · `feat/b5-rating-front` — **A95–A97** : état de notation servi sur les deux vues de deal (`rating`, `canRate` machine), écrans de notation réels (contexte sans moyenne — 2A, succès « révélée quand … », écran « fait », 409 → rechargement), carte « Noter {prénom} » partagée (terminé Expéditeur + Voyageur), listes et accueil sur `canRate`, profil public (badge de niveau + faits + prochain niveau, pouces des critères, « Signaler cet avis » mailto) ; deal 418 | ✅ mergée #146 |
| C · `feat/c1-admin-socle` — **D54 / A98–A101** : `apps/admin-ui` (3001, sans i18n), connexion admin en deux temps (`/auth/admin/*`, TOTP maison `@packages/totp` testé sur les vecteurs RFC, codes de secours, anti-rejeu, 5 échecs / 15 min), cookies `admin_*` séparés, `isAdminAuthenticated`, journal `AdminAction` en transaction (`@packages/admin-audit`), file « à arbitrer » + dossier complet (`GET /admin/disputes`, `/:id`) sans code de livraison, `grant-admin.ts` ; auth 80 · deal 424 ; smoke test réel sur bundles | ✅ mergée #148 |
| Fix · `fix/admin-login-diagnostic` + `fix/gateway-cors-admin-port` — recette C-PR1 : l'écran de connexion admin dit la vraie cause (code HTTP + message, 429 limiteur, réponse non JSON tolérée) ; CORS du gateway ouvert au port 3001 (localhost + LAN) — l'Origin du navigateur traverse le proxy D48 | ✅ mergées #150, #151 |
| C · `feat/c2-mediation` — **D55 / A102–A104** : version du Voyageur dans l'app (une fois, 72 h), décisions admin rejet / partiel / total et arbitrage de retenue (montants serveur, récapitulatif, motif ≥ 50), argent « remboursement puis exécuteur », machine DISPUTED → COMPLETED / CANCELLED (ADMIN), pas de notation après médiation, compteurs internes « litiges perdus », `booking.dispute_resolved` (écran + email + notification aux deux), écrans des deux rôles, admin-ui décision, seed `bzv-held` ; deal 440 ; smoke test réel (2 pièges Mongo attrapés) | ✅ mergée #153 |
| C · `feat/c3-admin-users` — **D56 / A105–A107** : profils admin (SUPER_ADMIN / MEDIATOR / SUPPORT / FINANCE, matrice dans le contrat), invitation d'un compte sans rôle client (lien 48 h), conflit d'intérêts serveur, fiche utilisateur complète et recherche, sanctions à deux niveaux (propose / applique / lève, effets par lecture, emails), sessions admin listées et révocables + alerte de connexion, Sentry (services + Next) ; auth 85 · deal 442 | ✅ mergée #155 |
| C · `feat/c4-admin-trips` — **D57 / A108–A110** : file « billets à vérifier » (motifs fermés, ouverture journalisée, expiration à la lecture, redépôt), « masqué par Yamba » (champ lu par la recherche, la page publique et la réservation ; proposition Support / exécution Médiateur ; email générique ; bandeau Voyageur), liste et fiche trajet admin, KPI d'accueil par permission (`/admin/kpis`, `/home`), OpenAPI admin trip-service, seed billet ; trip 202 · deal 443 · auth 87 | ✅ mergée #157 |
| C · `feat/c5a-admin-finances` — **D58 / A111–A113** : files d'exception (versements en échec, transferts renversés, retenues), fiche argent de tout deal (chronologie, identifiants, journal), rapprochement fournisseur en lecture seule (`PaymentProvider.inspect`, divergences codées, `refundId` stocké), rejeu manuel par l'exécuteur unique, clôture d'un renversement (re-verser avec nouvelle clé / abandonner), rejeux espacés sans plafond, KPI renversés, admin-ui `/finances` + `/deals/[id]`, OpenAPI, seed `bzv-reversed` ; deal 464 · auth 88 | ✅ mergée #159 |
| C · `feat/c5b-admin-finances-report` — **D58 5A / 3A-c, A114–A116** : rapport mensuel par devise (chaque fait à sa date, passifs du jour), export CSV journalisé (FINANCE, ≤ 366 j, formules neutralisées, BOM), remboursement manuel proposé (FINANCE / SUPPORT) et appliqué par SUPER_ADMIN (argent d'abord, verrou sur le cumul, `refund_issued` ADMIN), file « Remboursements proposés », portefeuille Expéditeur après COMPLETED, admin-ui `/finances/report` ; deal 475 · auth 89 | ✅ mergée #160 |
| C · `feat/c6a-admin-pilotage` — **D59 / A117–A119** : courbes serveur (semaine ISO / mois, périodes vides, petits multiples SVG, cache 60 s), corridors (trajets, demandes, taux, €/kg, litiges, vues, recherches, « demande sans offre »), chronologie complète d'un deal (quatre sources, whitelist, état de relais, journalisée), compteur de vues D5 (Redis dédoublonné, cartes + détail web), recherches par corridor ; correction OpenAPI deal-service (chemins admin replacés) ; trip 207 · deal 477 · auth 94 | ✅ mergée #162 |
| fix · `fix/session-expired-gate-provider` — fenêtre « session expirée » (A89) montée sous les providers (crash `useUiPreferences`), `turbopack.root` | ✅ mergée #163 |
| C · `feat/c6c-admin-pilotage-v2` — **D60 3A / 4A, A120–A122** : courbes agrandies (tableau, drill-down par point, borne 200, journal des inscriptions), onglet Finances du pilotage, pastille « n vues » + badge « Populaire » (cartes desktop / mobile, détail public) ; auth 96 | ✅ mergée #164 |
| C · `feat/c3bis-admin-roles` — **D60 1A, A123–A125** : profils cumulés (`adminRoles` liste + miroir `adminRole`, union des permissions, middleware / JWT / me, cases à cocher dans « Comptes admin », gardes inchangées, `grant-admin --roles`, `backfill-admin-roles.ts`) ; auth 99 | ✅ mergée #165 |
| admin-ui · `feat/pilotage-period-labels` — périodes en dates (« 29 juin → 5 juil. 2026 »), RG-PIL-11 | ✅ mergée #166 |
| C · `feat/c7a-admin-search-exports` — **D60 2A, A126–A128** : filtres serveur, tri, curseur (utilisateurs, trajets, billets, à arbitrer), exports CSV journalisés (opérationnels ids seulement ; nominatif SUPER_ADMIN + motif), lib `@packages/libs/csv`, bouton d'export encadré ; auth 103 · trip 209 · deal 478 | ✅ mergée #167 |
| F · `feat/f1-message-service` — **D61 / A132–A134** : message-service (6005), conversation par deal, rendez-vous objet (propose / accepte / contre-propose), gardes du fil (code de livraison refusé, coordonnées signalées), numéro révélé 2 h avant et tracé, réponses rapides FR/EN, relais outbox dédié (`messaging-events`, filtre `aggregateType`), OpenAPI 4e document, seed d'un fil ; message 14 · deal 478 | ✅ mergée #169 |
| F · `feat/f2-messaging-front` — **D61 / A135–A136** : messagerie du tableau de bord (liste, fil, rendez-vous, réponses rapides, numéro), bulle du header réelle, i18n `messaging` FR/EN, sondage adaptatif 3 s / 20 s, notification in-app (second consumer, topic et groupe dédiés) ; notification 83 | ✅ mergée #170 |
| F · `feat/f2b-deal-message-entry` — **A137** : les sept boutons « Message » / « Appeler » des écrans de deal ouvrent le fil du deal (`by-deal` + cache pré-rempli + `?conversation=`), « Appeler » = fil avec `?focus=phone` (bandeau numéro / heure d'ouverture), i18n `messaging.open.*` / `phone.banner.*` | ✅ mergée #171 |
| F · `feat/f3-messaging-admin` — **D61 6A/7A/8A, A138–A141** : relance email des non-lus (règle pure, verrou optimiste, sans le texte), lecture admin journalisée d'un fil (`conversations.read`), signalement d'un message (`Report` MESSAGE, file `/reports`, `reports.review`, KPI, décision + journal en transaction), purge à un an (règle pure, cron 03:30) ; message 28 | ✅ mergée #172 |
| fix · `fix/messaging-recette-i18n-assets` — recette 05/09 : clés `messaging.system` imbriquées (next-intl refuse le point dans une clé), `src/assets/.gitkeep` du message-service (le build webpack échouait, service jamais démarré → 500 gateway) | ✅ mergée #174 |
| CI · `chore/ci-build-i18n` — job « Build des services (webpack) » (six services) + règle « pas de point dans une clé » du script i18n ; 17 checks requis | ✅ mergée #177 |
| C · `feat/c8a-platform-settings` — **D62** : catalogue de 40 paramètres (classes A/B/C, portées métier / exploitation), document versionné, lib `@packages/libs/settings` (cache 30 s, repli sûr), rebranchement deal / trip / message, écriture `/admin/settings` (bornes, cohérence, 403 par portée, 409, journal par clé, email SUPER_ADMIN, reset avec diff), profil OPS, `GET /trips/pricing/params`, admin-ui Paramètres + documentation, `YAMBA-PARAMETRES.md` généré, `SiteConfig` supprimé ; message 33 · deal 489 · auth 121 | ✅ mergée #178 |
| C · `feat/c8b-gdpr` — **D63, A143** : sudo par code email, export JSON du membre (24 h), effacement immédiat bloqué par tout deal vivant (409 typé), anonymisation en une transaction (`ErasedAccount`, `DataRequest`), `isDeleted` respecté partout, tiers destinataire effacé à 30 j (cron), profil PRIVACY (registre, `users.erase`, export nominatif), préférence de relance, user-ui « Mes données », admin-ui `/privacy` ; auth 130 · deal 494 · message 36 | ✅ mergée #180 |
| C · `feat/c8c-maintenance` — **D64** : `/health` uniforme (lib partagée, Mongo + Redis, 2 s), battement des onze crons dans Redis, conservation chiffrée (`retention.*`, trois crons nocturnes, parqué jamais purgé), maintenance à deux interrupteurs (base journalisée + `MAINTENANCE_MODE`, lecture seule au gateway, bandeaux), page « État des services » + éditeur de maintenance ; auth 134 · notification 87 | ✅ mergée #182 |
| Lancement · `feat/d35-email-provider` — **D35** : `EmailProvider` (Resend par `fetch`, SMTP, faux refusé en production), webhook Svix (livré / rebond / plainte → `EmailDelivery`), liste de suppression respectée par tous les résolveurs + levée journalisée, trip-service sur la lib partagée, Mailpit ; notification 95 | ✅ mergée #185 |
| Lancement · `feat/d65-member-sessions` — **D65** (solde D27) : sudo à fenêtre de 15 min liée à la session (403 `SUDO_REQUIRED`, `SudoGate`, D63 migré), appareils connectés (libellé, IP, révocation), mot de passe et email (code à la nouvelle adresse, ancienne prévenue, autres sessions révoquées), Stripe sous sudo, écran Sécurité réel ; auth 138 | ✅ mergée #187 |
| Lancement · `feat/d66-posthog` — **D66** (D5) : PostHog Cloud EU, bannière opt-in (navigateur + `analyticsOptIn` + ConsentLog COOKIES), SDK chargé après accord, pages vues + funnel (search, trip, étapes, paiement, publication), identify par id, serveur par les événements outbox (liste blanche, uuid stable, fire-and-forget) ; notification 99 | ✅ mergée #189 |
| Lancement · `feat/e-profile-editable` — **D67** (chantier E) : profil tenu par le membre — `GET/PATCH /auth/me/profile`, avatar ImageKit `/avatars` (`fileId` gardé, ancien fichier supprimé, URL vérifiée), règle pure (noms 2–40, ≥ 16 ans, champs Voyageur), `profilePublic` (404 aux autres, `hidden` pour soi), `showCity`, écran Profil réel + « Voir mon profil public », slug immuable ; auth 142 | ✅ mergée #191 |
| Lancement · `feat/trust-report-wording` — **D68** (micro-PR confiance, lot 1, SIG-01…04) : `POST /reports` (trajet / membre par identifiant public, motifs fermés par cible, propre cible refusée, doublon 409, cible invisible 404), accusé email, file admin `/admin/reports` (décision + journal `REPORT_REVIEWED`, prioritaire à 3 ouverts, aucune sanction automatique), page `/reports` à deux files, KPI, modale générique sur les deux boutons inertes, wording D28, namespace `trips` mort retiré ; auth 153 | ✅ mergée #193 |
| F · `fix/messaging-quick-reply-draft` — **A142** : la réponse rapide remplit la zone de saisie sans envoyer (RG-FCH-23), suppression d'un message non retenue (RG-FCH-24) ; bulles invisibles sur téléphone (grille `minmax(0,1fr)` dès le mobile) ; front seul | ✅ mergée #175 |
| C · `feat/c6b-admin-alerts` — **D59 3A / 4A, A129–A131** : neuf règles de seuil (pures, seuils versionnés), `GET /admin/alerts` sans état, bandeau d'accueil, cron horaire dédoublonné par Redis (un email par règle et par jour, Redis injecté), `OPS_ALERTS_CRON_ENABLED` ; deal 485 | 🔄 PR ouverte |
...
| Indicateur | Valeur |
|---|---|
| Tests (plateforme) | 838 = trip 209 · deal 494 · notification 99 · message 36 (+ auth-service 153, check CI « Tests unitaires (auth-service) ») |
| Décisions au registre | D1 → D68 + D35 gravée (+ arbitrages A1 → A143) |
| Règles métier | ~50 (V2) + RG-B-01…35, RG-S-01…13, RG-C-01…16, RG-G-01…03, RG-D-01…16, RG-V-01…09, RG-F-01…06, RG-N-01…08, RG-T-01…06, RG-P-01…27, RG-A-01…08, RG-C-17…19, RG-FAV-01…06, RG-A-09…12, RG-C-21, RG-PAY-01…11, RG-LIT-01…07, RG-E-01…08, RG-VOY-01…07, RG-ANN-01…06, RG-FIN-01…05, RG-P-14, RG-H-01…05, RG-NOT-01…05, RG-NOTE-01…07 |
| PR mergées | #1 → #193 (#90 = pile B2-PR1/2/3 · #91 = docs post-merge · #92 = B2-PR4 emails · #93 = B2-PR5 tracker · #94 = docs post-merge · #95 = B3-PR1 serveur · #96 = B3-PR2 front · #97 = docs · #98 = B3-PR3 boîte du Voyageur · #99 = docs · #100 = B3-PR4 page demande · #101 = docs · #102 = typographie Deal · #103 = docs · #104 = fix ImageKit · #105 = docs · #106 = visionneuse photos · #108 = docs · #109 = fix relay outbox · #111 = allowedDevOrigins LAN · #112 = backlog inscription · #113 = atterrissage post-OTP · #114 = redirect inscription→connexion · #116 = fix recette auth · #117 = email-locale D44/D45 · #118 = modale de réservation A58 · #119 = favoris D46/A59 · #120 = pages auth D45/A60 · #121 = Google D47/A61 · #122 = « Rester connecté » A62 · #123 = connexion dans la fenêtre A63 · #124 = API même origine D48 · #125 = « Suivre » en modale A64 · #126 = docs suivi · #127 = B4-PR1 serveur D49–D52 · #128 = docs suivi · #129 = B4-PR2 tracker Expéditeur A71–A74 · #130 = docs suivi · #131 = B4-PR3 côté Voyageur A75–A78 · #132 = docs suivi · #133 = retenue ANN-01 D50/A79–A82 · #134 = docs suivi · #135 = Finances A83–A84 · #136 = docs suivi · #137 = fix jalons A85 · #138 = docs suivi · #139 = durcissement B4 A86–A89 · #140 = docs suivi · #141 = vocabulaire Voyageur A90 · #142 = notifications vivantes A91 · #143 = docs suivi · #144 = B5-PR1 notation D53 · #145 = docs suivi · #146 = B5-PR2 notation front A95–A97 · #147 = docs suivi · #148 = C-PR1 socle admin D54 · #149 = docs suivi · #150 = message de connexion admin · #151 = CORS port 3001 · #152 = docs suivi · #153 = C-PR2 médiation D55 · #154 = docs suivi · #155 = C-PR3 profils admin D56 · #156 = docs suivi · #157 = C-PR4 trajets et billets D57 · #158 = docs suivi · #159 = C-PR5a finances D58 · #160 = C-PR5b rapport / export / remboursement manuel · #161 = docs suivi · #162 = C-PR6a pilotage D59 · #163 = fix session expirée · #164 = C-PR6c pilotage v2 · #165 = C-PR3bis profils cumulés · #166 = périodes en dates · #167 = C-PR7a recherches / exports · #168 = C-PR6b alertes · #169 = F-PR1 message-service · #170 = F-PR2 messagerie front · #171 = F-PR2b boutons Message / Appeler · #172 = F-PR3 relance, admin, signalement, purge · #173 = docs suivi · #174 = fix recette messagerie (clés i18n, src/assets) · #175 = réponse rapide sans envoi A142 + bulles mobile · #176 = docs suivi · #177 = CI build + clés i18n · #178 = C-PR8a paramètres D62 · #179 = docs suivi · #180 = C-PR8b RGPD D63 · #181 = docs suivi · #182 = C-PR8c maintenance D64 · #183 = docs suivi · #184 = purge du contexte · #185 = D35 email Resend · #186 = docs suivi · #187 = D65 sessions membre · #188 = docs suivi · #189 = D66 PostHog · #190 = docs suivi · #191 = D67 profil éditable · #192 = docs suivi · #193 = D68 signalement + wording D28 — 13 checks (14 dès #148, 16 dès #171 : TypeScript admin-ui + message-service, tests message-service, 17 dès #177 : build webpack) comptés à chaque fois) |
| Documents | registre, spec, règles, 5 handoffs (dernier : SESSION-2026-08-28 + addendum 29/08), fiches PR (archive), 3 docs cumulatifs (technique, métier, apprentissage), `YAMBA-MOTEUR-PRIX.md/.pdf`, `YAMBA-PARAMETRES.md` (généré, D62), ce suivi |
...
3. ~~PR « paramètres serveur »~~ FAITE dans C-PR8a (D62, `GET /trips/pricing/params`) · cleanup legacy (0,5) + seeds (0,25) — entre deux lots.
4. Jalon 2 : ~~admin-ui (C)~~ **SOLDÉ le 05/09** · reste : ~~Sentry/PostHog, provider email (D35), sessions (SES-04/05), chantier E profil public~~ FAITS le 05/09 (#185, #187, #189, D67), moniteur externe de disponibilité (candidat D64), TrustScore D29-2 — **constitutif du lancement**.
