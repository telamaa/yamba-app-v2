...
### État au 3 septembre 2026 · `dev` = `3370efa` (#114) + `fix/auth-recette` en PR (recette auth 03/09, A50–A54, D44–D45 gravées) · 601 tests plateforme + 40 auth-service · `main` = `9c6e155` (#88)
...
| **Recette auth 03/09 — `fix/auth-recette`** (A50–A54) : critère de mot de passe NOMMÉ (front + codes serveur `PASSWORD_*`, `EMAIL_ALREADY_USED`), messages OTP construits des codes (plus d'anglais brut), barème OTP par paliers de 5 avec invalidation du code, fenêtre d'inscription 30 min prolongée au renvoi, « 5 minutes » → durée injectée, sujets OTP en français, œil centré, visuels manquants retirés ; +19 tests auth-service | ✅ PR ouverte |
