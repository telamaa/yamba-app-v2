...
| Lancement · `feat/trust-report-wording` — **D68** (micro-PR confiance, lot 1, SIG-01…04) : `POST /reports` (trajet / membre par identifiant public, motifs fermés par cible, propre cible refusée, doublon 409, cible invisible 404), accusé email, file admin `/admin/reports` (décision + journal `REPORT_REVIEWED`, prioritaire à 3 ouverts, aucune sanction automatique), page `/reports` à deux files, KPI, modale générique sur les deux boutons inertes, wording D28, namespace `trips` mort retiré ; auth 153 | 🟡 PR ouverte |
