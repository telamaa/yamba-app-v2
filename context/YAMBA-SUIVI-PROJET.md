...
| F · `feat/f2b-deal-message-entry` — **A137** : les sept boutons « Message » / « Appeler » des écrans de deal ouvrent le fil du deal (`by-deal` + cache pré-rempli + `?conversation=`), « Appeler » = fil avec `?focus=phone` (bandeau numéro / heure d'ouverture), i18n `messaging.open.*` / `phone.banner.*` | 🔄 PR ouverte #171 |
| F · `feat/f3-messaging-admin` — **D61 6A/7A/8A, A138–A141** : relance email des non-lus (règle pure, verrou optimiste, sans le texte), lecture admin journalisée d'un fil (`conversations.read`), signalement d'un message (`Report` MESSAGE, file `/reports`, `reports.review`, KPI, décision + journal en transaction), purge à un an (règle pure, cron 03:30) ; message 28 | 🔄 PR ouverte |
...
| Indicateur | Valeur |
|---|---|
| Tests (plateforme) | 791 = trip 209 · deal 485 · notification 83 · message 14 (+ auth-service 103, check CI « Tests unitaires (auth-service) ») |
| Décisions au registre | D1 → D61 (+ arbitrages A1 → A136) |
