...
| B5 · `feat/b5-rating-front` — **A95–A97** : état de notation servi sur les deux vues de deal (`rating`, `canRate` machine), écrans de notation réels (contexte sans moyenne — 2A, succès « révélée quand … », écran « fait », 409 → rechargement), carte « Noter {prénom} » partagée (terminé Expéditeur + Voyageur), listes et accueil sur `canRate`, profil public (badge de niveau + faits + prochain niveau, pouces des critères, « Signaler cet avis » mailto) ; deal 418 | 🔄 PR ouverte |
