...
| C · `feat/c8b-gdpr` — **D63, A143** : sudo par code email, export JSON du membre (24 h), effacement immédiat bloqué par tout deal vivant (409 typé), anonymisation en une transaction (`ErasedAccount`, `DataRequest`), `isDeleted` respecté partout, tiers destinataire effacé à 30 j (cron), profil PRIVACY (registre, `users.erase`, export nominatif), préférence de relance, user-ui « Mes données », admin-ui `/privacy` ; auth 130 · deal 494 · message 36 | 🔄 PR ouverte |
