...
| Lancement · `feat/d35-email-provider` — **D35** : `EmailProvider` (Resend par `fetch`, SMTP, faux refusé en production), webhook Svix (livré / rebond / plainte → `EmailDelivery`), liste de suppression respectée par tous les résolveurs + levée journalisée, trip-service sur la lib partagée, Mailpit ; notification 95 | 🔄 PR ouverte |
