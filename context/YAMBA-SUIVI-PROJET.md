...
| F · `feat/f1-message-service` — **D61 / A132–A134** : message-service (6005), conversation par deal, rendez-vous objet (propose / accepte / contre-propose), gardes du fil (code de livraison refusé, coordonnées signalées), numéro révélé 2 h avant et tracé, réponses rapides FR/EN, relais outbox dédié (`messaging-events`, filtre `aggregateType`), OpenAPI 4e document, seed d'un fil ; message 14 · deal 478 | 🔄 PR ouverte |
...
| Indicateur | Valeur |
|---|---|
| Tests (plateforme) | 786 = trip 209 · deal 485 · notification 78 · message 14 (+ auth-service 103, check CI « Tests unitaires (auth-service) ») |
| Décisions au registre | D1 → D61 (+ arbitrages A1 → A134) |
