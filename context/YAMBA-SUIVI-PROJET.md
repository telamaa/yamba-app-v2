...
| C · `feat/c3bis-admin-roles` — **D60 1A, A123–A125** : profils cumulés (`adminRoles` liste + miroir `adminRole`, union des permissions, middleware / JWT / me, cases à cocher dans « Comptes admin », gardes inchangées, `grant-admin --roles`, `backfill-admin-roles.ts`) ; auth 99 | 🔄 PR ouverte |
...
| Indicateur | Valeur |
|---|---|
| Tests (plateforme) | 762 = trip 207 · deal 477 · notification 78 (+ auth-service 99, check CI « Tests unitaires (auth-service) ») |
| Décisions au registre | D1 → D60 (+ arbitrages A1 → A125) |
