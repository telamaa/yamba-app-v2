...
| **D36** *(candidat — à graver avant le premier écran mobile)* | **Jalons 4 et 5 : applications Android et iOS sur UNE base React Native + Expo (TypeScript).** Jalon 4 = socle mobile + Android (Google Play) ; Jalon 5 = iOS (App Store). Réutilisation obligatoire : `@packages/pricing` (D34 : même devis que le web et le serveur), `@packages/api-contracts` + client généré depuis l'OpenAPI (D3), messages i18n JSON. Auth par tokens (refresh) — pas de cookies ; push via notification-service ; paiement Stripe Payment Sheet (Apple Pay / Google Pay natifs) ; caméra pour les photos de pickup. Pas de deuxième moteur, pas de deuxième contrat : le mobile est un client de plus des mêmes API et des mêmes règles | Deux bases natives = deux fois les bugs et les règles ; Expo donne les deux OS, l'OTA et EAS Build avec une équipe d'une personne ; le code métier est déjà en TypeScript partagé | Performances/animations natives limitées sur quelques écrans (acceptable pour un marketplace) ; dépendance à Expo | 🚪↔ (Expo vs bare RN) / 🚪→ (une seule base, mêmes API) |
