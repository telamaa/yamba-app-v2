...
| API-HOOK-01 | Écoute Stripe locale | majeure | secret affiché | CLI installée (1.50.10) et lancée avec la clé de test : « Ready! … whesc affiché » ; chaque événement transmis apparaît au journal avec le code rendu | OK | — |
| API-HOOK-02 | Quatre réponses Stripe | **bloquante** | 200/400/501/500 | **rejouée avec la CLI** : 200 sur `charge.succeeded` / `payment_intent.succeeded` / `payment_intent.created` (types non traités), 400 en-tête absent, 400 signature invalide, 501 sans secret ; le 500 transitoire est couvert par le test unitaire | OK | — |
| API-HOOK-03 | Cinq événements traités | majeure | effets attendus | **rejouée avec la CLI, autorisation Stripe RÉELLE** : intention `pi_3UDUp1…` confirmée en `requires_capture` (1450 c), deal créé, puis `stripe payment_intents cancel` → l'événement `payment_intent.canceled` arrive signé et le deal passe `CANCELLED` / `closedBy: SYSTEM` / `cancelReason: PAYMENT_AUTHORIZATION_LOST` · `account.updated` d'un compte connecté inconnu → 200 + avertissement | OK | — |
| API-HOOK-04 | Rejeu Stripe | **bloquante** | 1 seul effet | **rejouée avec la CLI** : `stripe events resend` ×2 sur le même `evt_…` → 200 aux trois livraisons, statut inchangé et **un seul** `booking.cancelled` en boîte d'envoi | OK | — |
