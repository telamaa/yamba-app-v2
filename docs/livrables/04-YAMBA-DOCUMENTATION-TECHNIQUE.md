...
1. **Infra** : `docker compose up -d` (Redpanda `yamba-redpanda` :9092 / :9644, Mailpit `yamba-mailpit` :1025 / :8026) puis, une fois, `./scripts/redpanda-bootstrap.sh` (topic `booking-events`, 12 partitions, rétention 7 j, `auto_create_topics_enabled=false` — le topic `messaging-events` se crée de la même façon). Mongo reste sur Atlas (distant, replica set) et Redis sur Upstash ou local.
2. **Dépendances** : `npm ci` (Node 22, npm 10.9.8), `npx prisma generate`, `npx prisma db push` si le schéma a changé.
3. **`.env`** : copier `.env.example`, remplir `DATABASE_URL`, `REDIS_DATABASE_URI`, les deux secrets JWT ; pour une recette locale : `EMAIL_PROVIDER=smtp`, `SMTP_HOST=localhost`, `SMTP_PORT=1025` (les emails se lisent sur `http://localhost:8026`) ; laisser `STRIPE_SECRET_KEY` vide pour jouer les parcours avec le fournisseur FAKE.
