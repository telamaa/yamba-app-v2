...
import FavoriteTripsList from "@/components/favorites/FavoriteTripsList";
...
export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  ...
  return { title: `${t("title")} — Yamba` };
}

/** D46 — « Mes favoris » : les trajets mis de côté, trajets passés inclus. */
export default async function FavoritesPage({ params }: { params: Promise<Params> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "favorites.page" });
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-6 lg:px-6 lg:py-8">
      <header className="mb-6">
        ...
        <p className="ml-12 text-sm leading-relaxed text-slate-600 dark:text-slate-400">{t("description")}</p>
      </header>
