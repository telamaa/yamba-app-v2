      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900">
        {intentError ? (
          <div className="space-y-3">
            <p className="text-[13px] text-slate-700 dark:text-slate-300">{intentError}</p>
            <button
              type="button"
          ...
          >
            <PaymentForm registerConfirmAction={registerConfirmAction} />
          </Elements>
        ) : (
...
function PaymentForm({ registerConfirmAction }: { registerConfirmAction: (fn: ConfirmPaymentFn | null) => void }) {
  ...
  return <PaymentElement options={{ layout: { type: "tabs", defaultCollapsed: false } }} />;
}
