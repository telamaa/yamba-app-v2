/**
 * BookingCompletedMobile.tsx — « Envoi terminé » (COMPLETED → VERIFIED), mobile (B4-PR2, A71)
 */
"use client";
...
export default function BookingCompletedMobile({ booking, onCloseAction }: Props) {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-white dark:bg-slate-950">
      ...
      <CompletedBanner booking={booking} compact />
      <div className="flex-1 space-y-3 px-4 pb-10 pt-4">
        ...
        <MediationDecisionNote booking={booking} compact />
        <PaymentReleasedCard booking={booking} compact />
        <DeliveryRecapCard booking={booking} compact />
        <DeliveredPaymentCard booking={booking} isConfirmed />
        <BookingCarrierCard booking={booking} compact />
        <RatingSoonNote booking={booking} compact />
        <button
          type="button"
