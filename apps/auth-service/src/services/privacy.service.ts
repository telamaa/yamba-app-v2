...
export function makePrivacyService(deps: {
  ...
  return {
    ...
    async eraseAccount(input: EraseInput): Promise<EraseResult> {
      ...
      const result = await withWriteConflictRetry(() => deps.db.$transaction(async (tx) => {
    ...
    async buildDataExport(userId: string): Promise<DataExport> {
      ...
      const [user, addresses, consents, carrierPage, trips, bookings, reviewsGiven, reviewsReceived, messages, meetups, phoneReveals, savedRoutes, favorites, following, notifications, reports, requests] = await Promise.all([
        ...
        db.booking.findMany({ where: { OR: [{ shipperId: userId }, { carrierId: userId }] }, select: { id: true, disputeTicket: true, shipperId: true, carrierId: true, status: true, requestedAt: true, acceptedAt: true, pickedUpAt: true, deliveredAt: true, completedAt: true, closedAt: true, cancelReason: true, trip: true, pricing: true, parcel: true, recipient: true, pickupPlace: true, deliveryPlace: true, payoutStatus: true, payoutAmountCents: true, refundAmountCents: true, retentionCents: true } }),
      ...
      return {
        ...
        bookings: bookings.map((b: Record<string, unknown> & { shipperId: string }) => ({
          id: b.id, disputeTicket: b.disputeTicket, role: role(b), status: b.status, requestedAt: b.requestedAt, acceptedAt: b.acceptedAt, pickedUpAt: b.pickedUpAt, deliveredAt: b.deliveredAt, completedAt: b.completedAt, closedAt: b.closedAt, cancelReason: b.cancelReason,
