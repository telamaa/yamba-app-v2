...
export function buildOpenApiDocument() {
  ...
  return {
    ...
    paths: {
      ...
      "/auth/refresh": { post: { tags: ["auth"], summary: "Rotate the session (refresh cookie or Bearer)", description: "Reads refresh_token from the cookie or from `Authorization: Bearer`, checks the jti in Redis, issues a new pair. The front replays queued requests after a 401 (api-client circuit breaker). With `x-token-delivery: body` (A201), no cookies are set and the rotated pair is returned in `tokens`.", operationId: "refreshTokens", responses: { "200": ok("RefreshedSessionResponse", "Session rotated"), "401": jsonResponse("UnauthorizedResponse", "Refresh token missing, revoked or expired"), "500": r500 } } },
      "/auth/logout": { post: { tags: ["auth"], summary: "Logout — revokes the refresh jti and clears cookies", description: "The refresh token is read from the cookie or from `Authorization: Bearer` (A201, cookie-less clients).", operationId: "logout", responses: { "200": okBody, "500": r500 } } },
      ...
      "/auth/me/sessions/all": { delete: member({ tags: ["me"], summary: "Revoke ALL sessions incl. the current one (D78 — cookies cleared)", operationId: "revokeMyAllSessions", responses: memberResponses({ "200": ok("RevokeSessionResponse", "Revoked count (incl. current)") }) }) },
      ...
      "/admin/audit": { get: admin("audit.read", { tags: ["admin"], summary: "Admin journal, newest first — filterable (A149)", description: "Filters on what Mongo indexes: period, author, action, target, IP. A malformed value is IGNORED, never an error; the response echoes the filters actually applied in `appliedFilters`. Free-text search on the detail stays in the browser, on the loaded rows.", operationId: "listAdminAudit", parameters: [cursorParam, q("from", { type: "string" }, "Début de période (date ou instant ISO)"), q("to", { type: "string" }, "Fin de période — une date seule inclut la journée entière"), q("adminUserId", ref("ObjectId"), "Auteur de l'action"), q("action", { type: "string" }, "Action exacte (ADMIN_ACTIONS)"), q("targetType", { type: "string" }, "USER, BOOKING, TRIP, SETTINGS…"), q("targetId", { type: "string", pattern: "^[A-Za-z0-9._:-]{1,100}$" }, "Identifiant de la cible : ObjectId, clé de paramètre, identifiant de session (ANO-ADM-74)"), q("ip", { type: "string" }, "Adresse IP exacte")], responses: adminResponses({ "200": ok("AdminAuditResponse", "Journal page") }) }) },
      "/admin/audit/authors": { get: admin("audit.read", { tags: ["admin"], summary: "Authors of the admin journal (A187) — every account that wrote at least one line, revoked admins included", operationId: "listAdminAuditAuthors", responses: adminResponses({ "200": inline("AdminAuditAuthors", { type: "object", properties: { items: { type: "array", items: { type: "object", properties: { id: { type: "string" }, name: { type: "string" }, active: { type: "boolean" } }, required: ["id", "name", "active"] } } }, required: ["items"] }) }) }) },
      "/admin/audit/export": { get: admin("exports.personal", { tags: ["admin"], summary: "CSV export of the FILTERED admin journal (A187) — audit.read AND exports.personal, reason ≥ 20, journaled EXPORTED", description: "Same filters as GET /admin/audit. Contains admin IPs and user agents (personal data). Capped at EXPORT_MAX_ROWS, X-Truncated says so.", operationId: "exportAdminAudit", parameters: [q("reason", { type: "string", minLength: 20 }, "Why this export", true), q("from", { type: "string" }, "Lower bound (ISO)"), q("to", { type: "string" }, "Upper bound (ISO)"), q("adminUserId", { type: "string" }, "Author"), q("action", { type: "string" }, "Action"), q("targetType", { type: "string" }, "Target type"), q("targetId", { type: "string" }, "Target id or key"), q("ip", { type: "string" }, "IP")], responses: adminResponses({ "200": { description: "text/csv (BOM, RFC 4180, formula prefixes neutralised)", content: { "text/csv": { schema: { type: "string" } } } }, "400": r400 }) }) },
      "/admin/settings": {
      ...
      "/admin/maintenance": {
        ...
        put: admin("maintenance.write", { tags: ["admin"], summary: "Switch the read-only mode (journaled, SUPER_ADMINs emailed)", description: "A181: lifting the read-only mode also clears the announcement; a new announcement date must be in the future (400 MAINTENANCE_SCHEDULE_IN_PAST). 409 STALE_VERSION (version lock, concurrent writes) or MAINTENANCE_FORCED_BY_ENVIRONMENT (A182: the gateway runs with MAINTENANCE_MODE=on).", operationId: "updateMaintenance", requestBody: jsonBody("UpdateMaintenanceRequest"), responses: adminResponses({ "200": ok("MaintenanceState", "State"), "400": r400, "409": jsonResponse("ErrorResponse", "Stale version or forced by the gateway environment") }) }),
      },
      ...
      "/admin/admins/invite": { post: admin("admins.manage", { tags: ["admin"], summary: "Invite an admin (D56) — email with a single-use token", operationId: "inviteAdmin", requestBody: jsonBody("InviteAdminRequest"), responses: adminResponses({ "200": ok("InviteAdminResponse", "Existing account: roles granted"), "201": ok("InviteAdminResponse", "Invited"), "400": jsonResponse("ErrorResponse", "Validation, ADMIN_ALREADY_GRANTED (also the loser of simultaneous invitations, A185), ACCOUNT_DELETED") }) }) },
      "/admin/admins/{id}": {
        patch: admin("admins.manage", { tags: ["admin"], summary: "Change the profiles of an admin (D60 1A: union of permissions)", operationId: "updateAdminRole", parameters: [idParam("id", "User id")], requestBody: jsonBody("UpdateAdminRoleRequest"), responses: adminResponses({ "200": ok("UpdateAdminRoleResponse", "Updated"), "400": r400, "403": jsonResponse("ErrorResponse", "ADMIN_IS_SELF, LAST_SUPER_ADMIN (A186: no other super administrator in service), or permission"), "404": r404 }) }),
        delete: admin("admins.manage", { tags: ["admin"], summary: "Revoke the admin profiles (sessions revoked; optional reason journaled, A189 b; security email without link, A189 c)", operationId: "revokeAdmin", parameters: [idParam("id", "User id")], requestBody: { required: false, content: { "application/json": { schema: ref("RevokeAdminRequest") } } }, responses: adminResponses({ "200": okOnly, "400": r400, "403": jsonResponse("ErrorResponse", "ADMIN_IS_SELF, LAST_SUPER_ADMIN (A186: no other super administrator in service), or permission"), "404": r404 }) }),
      },
      "/admin/admins/{id}/invite/resend": { post: admin("admins.manage", { tags: ["admin"], summary: "Resend a pending invitation (A189 a): new single-use link, the previous one dies", operationId: "resendAdminInvite", parameters: [idParam("id", "User id")], responses: adminResponses({ "200": ok("ResendAdminInviteResponse", "New link sent"), "400": r400, "404": r404, "409": jsonResponse("ErrorResponse", "ADMIN_INVITE_NOT_PENDING (accepted, revoked or deleted)") }) }) },
      ...
      "/admin/users/{id}/communications": { get: admin("users.read", { tags: ["admin"], summary: "Member communications (D79) — last in-app notifications and email deliveries, types and statuses only, never a body nor the delivery code; send errors are redacted; read journaled USER_VIEWED (coalesced)", operationId: "getAdminUserCommunications", parameters: [idParam("id", "User id")], responses: adminResponses({ "200": ok("AdminUserCommunicationsResponse", "Communications"), "404": r404 }) }) },
      ...
      "/admin/users/{id}/email-suppression": { delete: admin("users.email.unsuppress", { tags: ["admin"], summary: "Lift an email suppression after a bounce / complaint (D35 4A, journaled) — reason required (A155), conditional write", operationId: "unsuppressEmail", parameters: [idParam("id", "User id")], requestBody: jsonBody("UnsuppressEmailRequest"), responses: adminResponses({ "200": okOnly, "400": r400, "404": r404 }) }) },
      ...
      "/admin/users/{id}/erasure-blockers": { get: admin("users.erase", { tags: ["admin"], summary: "GDPR — why this member cannot be erased yet (A179)", description: "Read before the erasure click; not journaled. The same check runs again inside the erasure transaction.", operationId: "adminErasureBlockers", parameters: [idParam("id", "User id")], responses: adminResponses({ "200": ok("ErasureCheckResponse", "Blockers (empty = erasable now)"), "400": r400, "404": r404 }) }) },
      "/admin/privacy/requests": { get: admin("privacy.requests.read", { tags: ["admin"], summary: "GDPR requests register (exports, erasures) — journaled read", operationId: "listDataRequests", parameters: [cursorParam, { name: "userId", in: "query", required: false, schema: ref("ObjectId"), description: "Only this member's requests (A179)" }], responses: adminResponses({ "200": ok("DataRequestsResponse", "Requests"), "400": r400 }) }) },
