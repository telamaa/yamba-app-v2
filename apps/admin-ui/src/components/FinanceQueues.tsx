              ...
              {data.items.map((it) => (
                <tr key={it.bookingId} className="border-t border-slate-100 align-top hover:bg-slate-50">
                  ...
                  <td className="px-3 py-2 text-[12.5px]">
                    {kind === "HELD" ? "retenue conservée" : kind === "PROPOSED_REFUNDS" ? (
                      <>remboursement proposé{it.proposalStale && <span className="ml-1 rounded-full bg-red-50 px-2 py-0.5 text-[10px] font-semibold text-red-700" title="La proposition dépasse ce qui reste remboursable, ou le deal ne peut plus rien recevoir.">caduque</span>}</>
                    ) : (
                      <>
